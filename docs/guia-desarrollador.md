...
|---|---|---|
| Hero (`src/sections/hero.tsx`) | Un `<img>` en el primer paint; **no se desmonta**. Vecinos después de LCP. Ken Burns en `.hero-kenburns`, no en el `img`. El primer retrato no zoom-ea (el lab mide un still). Carrusel: 8s de hold, después 6s. Preload en `page.tsx`. Mobile tope 960×1510. | Lighthouse toma el `img` como LCP. Si el nodo sale del DOM, pasa a opacity 0 o el padre hace `scale` **mientras el lab corre** (~7–10s en 4G lento), el informe vuelve `NO_LCP` aunque el filmstrip sí muestre la cara. |
